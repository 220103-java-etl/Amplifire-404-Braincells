...
function gettingCharts(theChartId,barHeight,barLabels){
let AgentBar=document.getElementById(theChartId).getContext('2d');
let myChart= new Chart(AgentBar,{
    type:'bar',
    data:{
        labels:barLabels,
        datasets:[{
...
function getData(url,methods,Destid) {
    ...
    let har = new XMLHttpRequest();

    har.onreadystatechange = receiveData;

    har.open(methods, url, true)
    har.send();

    function receiveData() {

        if (har.readyState == 4 && har.status == 200) {
            let h = har.responseText;
            ...
            hJson = JSON.parse(h);
            let greaterThanvalue=document.getElementById('agent_rating')
            valueSelected=greaterThanvalue.value
            let stateValue=document.getElementById('state selection')
            let barHeightArray=[]
            let allStateAccepatance= new Map;
            ...
            if(valueSelected<=hJson[h]['AgentRating']){




            if(hJson[h]['approval']==true){
                trueVals++



                allStateAccepatance.set(hJson[h]['state'],allStateAccepatance.get(hJson[h]['state'])+1)
            }else{
                falseVals++
                allFromStateDenial.set(hJson[h]['state'],allFromStateDenial.get(hJson[h]['state'])+1)
            }
                }
            }else{

                if(valueSelected<=hJson[h]['agentRating']&&stateValue.value==hJson[h]['state']){
            ...
            canvas3.setAttribute('id','AgentBar')
            div3.append(canvas3);
            gettingCharts('AgentBar',barHeightArray,states);
            }
...
let states=['Alabama','Alaska','Arizona','Arkansas','California','Colorado','Connecticut','Delaware','Florida','Georgia','Hawaii','Idaho','Illinois','Indiana','Iowa','Kansas','Kentucky','Louisiana','Maine','Maryland','Massachusetts','Michigan','Minnesota','Mississippi','Missouri','Montana','Nebraska','Nevada','New Hampshire','New Jersey','New Mexico','New York','North Carolina','North Dakota','Ohio','Oklahoma','Oregon','Pennsylvania','Rhode Island','South Carolina','South Dakota','Tennessee','Texas','Utah','Vermont','Virginia','Washington','West Virginia','Wisconsin','Wyoming'];
function gettingCharts(theChartId,barHeight,barLabels){
let ClaimBar=document.getElementById(theChartId).getContext('2d');
let myChart= new Chart(ClaimBar,{
    type:'bar',
    data:{
        labels:barLabels,
        datasets:[{
...
function getData(url,methods,Destid) {
    ...
    let cca = new XMLHttpRequest();

    cca.onreadystatechange = receiveData;

    cca.open(methods, url, true)
    cca.send();

    function receiveData() {

        if (cca.readyState == 4 && cca.status == 200) {
            let c = cca.responseText;
            ...
            cJson = JSON.parse(c);
            let greaterThanvalue=document.getElementById('claim_reason')
            valueSelected=greaterThanvalue.value
            let stateValue=document.getElementById('state selection')
            let barHeightArray=[]
            let allStateAccepatance= new Map;
            ...
            if(valueSelected<=cJson[c]['ClaimReason']){




            if(cJson[c]['approval']==true){
                trueVals++



                allStateAccepatance.set(cJson[c]['state'],allStateAccepatance.get(cJson[c]['state'])+1)
            }else{
                falseVals++
                allFromStateDenial.set(cJson[c]['state'],allFromStateDenial.get(cJson[c]['state'])+1)
            }
                }
            }else{

                if(valueSelected<=cJson[c]['claimReason']&&stateValue.value==cJson[c]['state']){
            ...
            let div4=document.getElementById('divForCanv4')
            div4.innerHTML=""
            let canvas4=document.createElement('canvas')
            canvas4.setAttribute('id','ClaimBar')
            div4.append(canvas4);
            gettingCharts('ClaimBar',barHeightArray,states);
